...
# Load deck.csv (robust: cwd -> sys.path -> Kaggle layout; cabt execs agents
# without __file__ and reads the deck at module-load time, before any chdir).
def _deck_path():
    for p in ["."] + list(sys.path) + ["/kaggle_simulations/agent"]:
        if p and os.path.exists(os.path.join(p, "deck.csv")):
            return os.path.join(p, "deck.csv")
    return "deck.csv"
...
# === S1.3 hardening: never crash, always return a legal selection ===
_orig_agent = agent

def _legal_fallback(obs_dict):
    """Return a legal selection when the main policy raises."""
    try:
        obs = to_observation_class(obs_dict)
    except Exception:
        return [0]
...
def agent(obs_dict):  # type: ignore[no-redef]
    try:
